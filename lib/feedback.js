...
(() => {
  ...
  const state = {
    ...
    sse: null,
    sseRetry: 0,
    pages: [],               // [{name, open, addressed, total}]
    pagesOpen: false,        // dropdown visibility
  };
  ...
  // Pages dropdown — populated by renderPagesDropdown().
  const pagesBtnLabel = h('span', { class: 'hfb-pages-label' }, '📄 Pages (0)');
  const pagesOtherDot = h('span', { class: 'hfb-pages-other-dot', style: { display: 'none' } });
  const pagesBtn = h('button', {
    class: 'hfb-btn hfb-btn-sm hfb-pages-button',
    title: 'Switch to another page',
    onclick: (e) => { e.stopPropagation(); togglePagesDropdown(); },
  }, pagesBtnLabel, pagesOtherDot);
  const pagesDropdown = h('div', { class: 'hfb-pages-dropdown' });
  const pagesWrap = h('div', { class: 'hfb-pages-wrap' }, pagesBtn, pagesDropdown);
  ...
  const sendComment = async (comment, retry = false) => {
    ...
    try {
      ...
      toast('Comment sent');
      // Fire-and-forget visual-context capture. Failures don't affect the comment.
      captureAndUploadScreenshot(comment).catch(() => {});
    } catch (e) {
  ...
  // ─── Visual context: capture a PNG of the region for the comment ───
  const captureAndUploadScreenshot = async (comment) => {
    if (typeof html2canvas === 'undefined') return; // vendor not loaded
    const anchor = comment.anchor || {};
    let captureOpts;
    if (anchor.rect && (anchor.type === 'text' || anchor.type === 'element')) {
      const pad = 80;
      const docW = document.documentElement.scrollWidth;
      const docH = document.documentElement.scrollHeight;
      const vpW = window.innerWidth;
      const vpH = window.innerHeight;
      let x = Math.max(0, anchor.rect.x - pad);
      let y = Math.max(0, anchor.rect.y - pad);
      let w = anchor.rect.w + 2 * pad;
      let h = anchor.rect.h + 2 * pad;
      // If the rect is bigger than the viewport, clamp to the current viewport.
      if (w > vpW || h > vpH) {
        x = window.scrollX;
        y = window.scrollY;
        w = vpW;
      ...
      captureOpts = { x, y, width: w, height: h };
    } else {
      // page anchor or no rect: capture current viewport
      captureOpts = {
        x: window.scrollX,
        y: window.scrollY,
    ...
    // Tiny indicator while we capture.
    let indicator = null;
    try {
      indicator = h('div', { class: 'hfb-toast hfb-visible' }, '📸 capturing context…');
      root.appendChild(indicator);
    } catch (_) { /* indicator is optional */ }
    try {
      // Hide our own UI so the sidebar/FAB/etc. don't appear in the screenshot.
      ...
      // Downscale if too wide to keep payload reasonable.
      ...
      const res = await api('/api/screenshot', {
        method: 'POST',
        body: JSON.stringify({ comment_id: comment.id, image_b64: b64 }),
      });
      if (!res.ok) throw new Error('HTTP ' + res.status);
    } catch (e) {
      // Best-effort: don't surface errors aggressively. The comment is already saved.
      // eslint-disable-next-line no-console
      console.warn('[hfb] screenshot capture failed', e);
      toast('Screenshot capture failed (comment saved)');
    } finally {
  ...
  // ─── Pages dropdown ───
  const pageHref = (name) => {
    const u = new URL('/' + encodeURIComponent(name), location.origin);
    if (TOKEN) u.searchParams.set('t', TOKEN);
    return u.toString();
  };

  const updatePagesButton = () => {
    // Find this page's open count from state.pages; fall back to local state if missing.
    const here = state.pages.find(p => p.name === PAGE);
    const localOpen = state.comments.filter(c => c.status !== 'addressed' && !c.parent_id).length;
    const openHere = here ? here.open : localOpen;
    const otherHasOpen = state.pages.some(p => p.name !== PAGE && p.open > 0);
    pagesBtnLabel.textContent = `📄 Pages (${openHere})`;
    pagesOtherDot.style.display = otherHasOpen ? '' : 'none';
  };

  const renderPagesDropdown = () => {
    pagesDropdown.innerHTML = '';
    if (state.pages.length === 0) {
      pagesDropdown.appendChild(h('div', { class: 'hfb-page-empty' }, 'No pages found'));
      return;
    }
    for (const p of state.pages) {
      const isCurrent = p.name === PAGE;
      const countsText = `${p.open} open • ${p.addressed} addressed`;
      const nameSpan = h('span', { class: 'hfb-page-name' },
        isCurrent ? h('span', { class: 'hfb-page-dot' }, '● ') : null,
        p.name,
      );
      const countsSpan = h('span', { class: 'hfb-page-counts' }, countsText);
      if (isCurrent) {
        pagesDropdown.appendChild(h('div', {
          class: 'hfb-page-row hfb-page-current',
          title: 'Current page',
        }, nameSpan, countsSpan));
      } else {
        pagesDropdown.appendChild(h('a', {
          class: 'hfb-page-row',
          href: pageHref(p.name),
  ...
  const togglePagesDropdown = (force) => {
    const next = typeof force === 'boolean' ? force : !state.pagesOpen;
    state.pagesOpen = next;
    pagesDropdown.classList.toggle('hfb-visible', next);
    if (next) renderPagesDropdown();
  };
  ...
  const loadPages = async () => {
    try {
      ...
      updatePagesButton();
      if (state.pagesOpen) renderPagesDropdown();
    } catch (_) { /* ignore */ }
  };
  ...
  document.addEventListener('click', (e) => {
    ...
    if (pagesWrap.contains(e.target)) return;
    ...
    try {
      ...
      es.addEventListener('inbox', (ev) => {
        try {
          ...
          // Any inbox event (any page) may change cross-page counters; refresh.
          loadPages();
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'Escape') {
      ...
      if (state.pagesOpen) { togglePagesDropdown(false); return; }
